...
static void OLED_I2C_Delay(void)
{
	...
	for (i = 0U; i < OLED_I2C_DELAY_CYCLES; i++)
	{
		__NOP();
	}
}
...
static void OLED_WritePageData(u8 page)
{
	...
	for (n = 0U; n < 128U; n++)
	{
		page_buf[n] = OLED_GRAM[n][page];
	}

	OLED_I2C_SendWithControl(0x40U, page_buf, 128U, (u8)OLED_I2C_ADDR);
#if OLED_I2C_TRY_ALT_ADDR
	OLED_I2C_SendWithControl(0x40U, page_buf, 128U, (u8)OLED_I2C_ADDR_ALT);
#endif
}
...
#if OLED_I2C_TRY_ALT_ADDR
	I2C_Start();
	Send_Byte((u8)OLED_I2C_ADDR_ALT);
	I2C_WaitAck();
	if(mode){Send_Byte(0x40);}
  else{Send_Byte(0x00);}
	I2C_WaitAck();
